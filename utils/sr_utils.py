...
def load_LR_HR_imgs_sr(fname, imsize, factor, enforse_div32=None):
    ...
    if enforse_div32 == 'CROP':
        new_size = (img_orig_pil.size[0] - img_orig_pil.size[0] % 32, img_orig_pil.size[1] - img_orig_pil.size[1] % 32) # PIL格式形状[w,h,channel],numpy格式形状[w,h,channel]

        bbox = [(img_orig_pil.size[0] - new_size[0])/2, (img_orig_pil.size[1] - new_size[1])/2,(img_orig_pil.size[0] + new_size[0])/2,(img_orig_pil.size[1] + new_size[1])/2,]
        
        # 裁剪后原图的PIL和numpy格式(只有crop的方式保持分辨率不变)：
    ...
    return {'orig_pil': img_orig_pil,'orig_np':  img_orig_np,'LR_pil':  img_LR_pil, 'LR_np': img_LR_np,'HR_pil':  img_HR_pil, 'HR_np': img_HR_np}
